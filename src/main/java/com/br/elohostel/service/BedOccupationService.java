...
public class BedOccupationService {
    ...
    public List<Bed> findAvailableBedsInRoom(Room room, Set<LocalDate> dates) {
        return bedRepository.findBedsInRoomByStatus(room.getId(), BedStatus.AVAILABLE, dates, getCurrentTenantKey());
    }

    public List<Bed> findAvailableBedsInRoomAlternative(Room room, Set<LocalDate> dates) {
        List<Bed> unoccupiedBeds = bedRepository.findUnoccupiedBedsInRoom(room.getId(), dates, getCurrentTenantKey());
    ...
    public List<Bed> findAvailableAndUnoccupiedBeds(Room room, Set<LocalDate> dates) {
        return bedRepository.findAvailableAndUnoccupiedBeds(room.getId(), dates, getCurrentTenantKey());
    }
    ...
    public boolean isBedAvailableForDates(Bed bed, Set<LocalDate> dates) {
        return !bedOccupationRepository.existsByBedIdAndOccupiedDaysIn(bed.getId(), dates, getCurrentTenantKey());
    }
    ...
    public BedOccupation createBedOccupation(Bed bed, Reserve reserve, Set<LocalDate> occupiedDays) {
        ...
        occupation.setTenant(getCurrentTenant());
    ...
    public void removeExistingBedOccupations(Reserve reserve) {
        List<BedOccupation> existingOccupations = bedOccupationRepository.findByReserveAndTenant_TenantKey(reserve, getCurrentTenantKey());  
    ...
    public List<BedAvailabilityInfo> getBedAvailabilityByRoomAndDates(Set<LocalDate> dates) {
        List<Room> allRooms = roomRepository.findByTenant_TenantKey(getCurrentTenantKey());
    ...
    public List<BedOccupation> findOccupationsInPeriod(LocalDate startDate, LocalDate endDate) {
        return bedOccupationRepository.findByOccupiedDaysBetween(startDate, endDate, getCurrentTenantKey());
    }
    ...
    public void releaseBedsByCheckout(Date checkOutDate) {
        ...
        List<BedOccupation> occupationsToRelease = bedOccupationRepository
            .findByOccupiedDaysContaining(localCheckOut, getCurrentTenantKey());
    ...
    public Map<String, Object> getBedOccupationStatistics(LocalDate startDate, LocalDate endDate) {
        ...
        Long totalBeds = bedRepository.findByBedStatusAndTenant_TenantKey(null, getCurrentTenantKey()).stream().count();
